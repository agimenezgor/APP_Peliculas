    ...
    mapArray() {
        return <React.Fragment>
            {this.props.renderArray.map((movie, index) => (
                <div id={movie.id} className="col-md-3 mb-5" key={index}>
                    <Link to="/pelicula">
                        <button id={movie.id} onClick={this.props.changeMovieDetail} className="border border-primary bg-secondary m-5 p-3 rounded-lg d-flex flex-column align-items-center justify-content-center" style={{maxHeight: '60vh', minHeight: '60vh', cursor: 'pointer'}}>
